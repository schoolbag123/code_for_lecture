...
def discriminator(seed=None):
    ...
    model = nn.Sequential(nn.Flatten(),nn.Linear(784,256),nn.LeakyReLU(0.01),nn.Linear(256,256),nn.LeakyReLU(0.01),nn.Linear(256,1))
...
def discriminator_loss(logits_real, logits_fake):
    ...
    lossfake = bce_loss(logits_fake, torch.zeros_like(logits_fake).type(dtype))
    losstrue = bce_loss(logits_real, torch.ones_like(logits_real).type(dtype))
    loss = lossfake + losstrue
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    loss = 0.5 * (scores_real-1)**2 + scores_fake**2 * 0.5 
...
def build_dc_classifier(batch_size):
    ...
    return nn.Sequential(Unflatten(batch_size, 1, 28, 28),
                         nn.Conv2d(1,32,(5,5),1),nn.LeakyReLU(0.01),nn.MaxPool2d((2,2),2),
                         nn.Conv2d(32,64,(5,5),1),nn.LeakyReLU(0.01),nn.MaxPool2d((2,2),2),
                         nn.Flatten(),nn.Linear(4*4*64,4*4*64),nn.LeakyReLU(0.01),nn.Linear(4*4*64,1)
                        )
...
def build_dc_generator(noise_dim=NOISE_DIM):
    ...
    return nn.Sequential(nn.Linear(noise_dim,1024),nn.ReLU(),nn.BatchNorm1d(1024),
                         nn.Linear(1024,7*7*128),nn.ReLU(),nn.BatchNorm1d(7*7*128),
                         Unflatten(-1,128,7,7),
                         nn.ConvTranspose2d(128,64,(4,4),2,1),nn.ReLU(),nn.BatchNorm2d(64),
                         nn.ConvTranspose2d(64,1,(4,4),2,1),nn.Tanh(),
                         Flatten()
                        )
