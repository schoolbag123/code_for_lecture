...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        features = self.visual_projection(features).unsqueeze(1)   #唉唉，这里要调整这个形状
        captions_em = self.positional_encoding(self.embedding(captions))
        #tgt_mask = torch.tril(captions_em)

        tgt_mask = torch.tril(torch.ones(T, T,
                                         device=captions_em.device,
                                         dtype=captions_em.dtype))  ##没想到这里这么复杂，mask是01表示，并且要注意设备问题)

        features_out = self.transformer(tgt=captions_em, memory=features, tgt_mask=tgt_mask)
