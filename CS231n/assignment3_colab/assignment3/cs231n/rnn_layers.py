...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    next_h_ = x @ Wx + prev_h @ Wh + b
    next_h = np.tanh(next_h_)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    T = x.shape[1]
    next_h = h0
    h = np.zeros((T,x.shape[0],h0.shape[1]))
    #print(x.shape,_x.shape,next_c.shape,next_h.shape,_x[0].shape)
    cache = []
    for i in range(0,T):
        next_h, cache1 = rnn_step_forward(_x[i],next_h,Wx,Wh,b)
        h[i] = next_h
        cache.append(cache1)
    cache.append(h0.shape[1])
...
def rnn_backward(dh, cache):
    ...
    dh_ = np.swapaxes(dh,0,1)

    dprev_c_ = np.ones_like(dh_[0])

    for i in range(T-1,-1,-1):
        cache1 = cache[i]
        dx_, dh0, dWx_, dWh_, db_ = rnn_step_backward(dh_[i] + dh0,cache1)
        dx[i] += dx_
        dWx += dWx_
        dWh += dWh_
        db += db_
...
def word_embedding_backward(dout, cache):
    ...
    D = W.shape[1]
    dW = np.zeros((V,D))
    np.add.at(dW,x,dout)  #唉唉，别人的代码写的还是比我好，我就没搞明白这个at如何加速的（在看到别人代码之前）
                          ## 唉唉，原来是dW = 巴拉巴拉，没想到这个函数直接调用即可，不能dw = ....
                          ## 修bug时间喜提10分钟+++
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    cache = x, prev_h, prev_c, Wx, Wh, b, next_h, next_c, f, i, g,o
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    x, prev_h, prev_c, Wx, Wh, b, next_h, next_c,f,i,g,o= cache
    D = x.shape[1]
    ...
    #print(dao.shape)
    #print(x.shape)
    db[2*H:3*H] = np.sum(dao,axis=0).reshape((H,))
    #print(dWx[:,2*H:3*H].shape)
    dWx[:,2*H:3*H] = x.T @ dao
    #print(dx.shape)
    dx += dao @ Wx[:,2*H:3*H].T
    #print(dprev_h.shape)
    dprev_h += dao @ Wh[:,2*H:3*H].T
    #print(dWh[:,2*H:3*H].shape)
    #print(dao.shape,prev_h.shape)
    dWh[:,2*H:3*H] = prev_h.T @ dao

    dnext_c += dnext_h * o * (1 - np.square(np.tanh(next_c))) #计算图不熟悉导致滴
    ...
    db[1*H:2*H] = np.sum(daf,axis=0).reshape((H,))
    dWx[:,1*H:2*H] = x.T @ daf
    dx += daf @ Wx[:,1*H:2*H].T
    dprev_h += daf @ Wh[:,1*H:2*H].T
    dWh[:,1*H:2*H] = prev_h.T @ daf    #反正遇到矩阵乘法,不动上游导数，对另一个用.T,用完以后输出一下格式，根据形状试前后顺序即可

    db[:H] = np.sum(dai,axis=0).reshape((H,))
    dWx[:,:H] = x.T @ dai
    dx += dai @ Wx[:,:H].T
    dprev_h += dai @ Wh[:,:H].T
    dWh[:,:H] = prev_h.T @ dai

    db[3*H:] = np.sum(dag,axis=0).reshape((H,))
    dWx[:,3*H:] = x.T @ dag
    dx += dag @ Wx[:,3*H:].T
    dprev_h += dag @ Wh[:,3*H:].T
    dWh[:,3*H:] = prev_h.T @ dag
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    cache = []
    for i in range(0,T):
        next_h, next_c, cache1 = lstm_step_forward(_x[i],next_h,next_c,Wx,Wh,b)
        h[i] = next_h
        cache.append(cache1)
    cache.append(h0.shape[1])
    cache.append(x.shape)
...
def lstm_backward(dh, cache):
    ...
    dWh = np.zeros((H,4*H))
    db = np.zeros(4*H,)
    dc = np.zeros((N,H),dtype=np.float64)
    
    dh_ = np.swapaxes(dh,0,1)

    dprev_c_ = np.ones_like(dh_[0])
    dprev_h_ = np.zeros_like(dprev_c_)

    for i in range(T-1,-1,-1):
        cache1 = cache[i]
        dx_, dh0, dc, dWx_, dWh_, db_ = lstm_step_backward(dh_[i] + dh0 ,dc,cache1)
        dx[i] += dx_
        dWx += dWx_
        dWh += dWh_
        db += db_
