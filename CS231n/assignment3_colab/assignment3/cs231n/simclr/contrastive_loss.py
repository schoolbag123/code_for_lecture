...
def sim(z_i, z_j):
    ...
    norm_dot_product = (z_i @ z_j.T) / (torch.linalg.norm(z_i,ord=2)* torch.linalg.norm(z_j,ord=2))
...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        for i in range(0,2*N):
            if i != k:
                total1 += torch.exp(sim(z_k,out[i]) / tau)
            if i != k+N:
                total2 += torch.exp(sim(z_k_N,out[i]) / tau)

        target = torch.exp(sim(z_k,z_k_N) / tau)
        total_loss -= torch.log(target / total1) + torch.log(target / total2)
...
def sim_positive_pairs(out_left, out_right):
    ...
    # print((torch.linalg.norm(out_left,ord=2,dim=1,keepdim = True)* torch.linalg.norm(out_right,ord=2,dim=1,keepdim = True)).shape)
    # print((out_left @ out_right.T).shape)

    # pos_pairs = (out_left @ out_right.T).sum(dim =1) / (torch.linalg.norm(out_left,ord=2,dim=1,keepdim = True)* torch.linalg.norm(out_right,ord=2,dim=1,keepdim = True))
    ##麻了，不晓得为什么这里数值有点问题，应该是计算顺序导致的精度损失不一样，数学上的表达肯定是等价的
    ##只能用用其他人的实现了
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    pos_pairs = torch.exp(sim_positive_pairs(out_left,out_right) / tau)
    ...
    numerator = torch.cat((pos_pairs,pos_pairs),dim = 0) / denom.reshape((denom.shape[0],1))
    ...
    loss = -torch.log(numerator).sum() / (2 * N)
