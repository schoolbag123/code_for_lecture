...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        ### YOUR CODE HERE ###
        data = self.data[idx]
        maxcutlength = min(int(self.block_size*7/8),len(data))
        truecutlen = random.randint(4, maxcutlength)
        beginindx = random.randint(0,len(data)-truecutlen)
        curstring = data[beginindx:beginindx+truecutlen+1]
        curstring = list(curstring)
        lentocut = random.betavariate(1, 3)
        beginindxinstring = int((len(curstring) * lentocut) / 2)
        lastindexinstring = int(len(curstring) - (len(curstring) * lentocut) / 2)
        maskedstring = curstring[beginindxinstring:lastindexinstring]
        curstring[beginindxinstring:lastindexinstring] = self.MASK_CHAR
        curstring = curstring + [self.MASK_CHAR] + maskedstring
        curstring = curstring + [self.PAD_CHAR] * (self.block_size + 1 - len(curstring))
        x = curstring[:-1]
        y = curstring[1:]
