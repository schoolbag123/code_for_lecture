...
def precompute_rotary_emb(dim, max_positions):
    ...
    # print(f"the shape of rope_cache:{rope_cache.shape}")
    theta_i = [ 10000**(-2 * (i-1)/dim) for i in range(1, dim//2 + 1) ]
    theta_i = torch.tensor(theta_i)
    # print(f"the shape of theta_i:{torch.tensor(theta_i).shape}")
    ttheta_i = torch.zeros((max_positions,int(dim/2)))
    for i in range(0,max_positions):
        ttheta_i[i] = i*theta_i
    # print(f"the shape of ttheta_i:{ttheta_i.shape}")
    # flatlist = [item for sublist in ttheta_i for item in sublist]
    # flatlist = torch.tensor(flatlist)
    # print(f"the shape of flatlist:{flatlist.shape}")
    rope_cache[:,:,0] = torch.cos(ttheta_i)
    rope_cache[:,:,1] = torch.sin(ttheta_i)
    ### END YOUR CODE ###
    return rope_cache
...
def apply_rotary_emb(x, rope_cache):
    ...
    rotated_x = None
    ### YOUR CODE HERE ###
    comropecache = torch.view_as_complex(rope_cache[0:x.shape[2],:x.shape[3]//2,:])
    comropecache = comropecache.reshape((1,1,comropecache.shape[0],comropecache.shape[1]))
    rex = x.reshape((x.shape[0],x.shape[1],x.shape[2],x.shape[3]//2,2))
    comx = torch.view_as_complex(rex)
    # print(f'the shape of comx is {comx.shape}')
    # print(f'the shape of rope_cache is {rope_cache.shape}')
    # print(f'the shape of comropecache is {comropecache.shape}')
    comans = comropecache * comx
    rotated_x = torch.view_as_real(comans).reshape_as(x)
