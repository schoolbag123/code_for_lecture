...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        elif transition == "LA" or transition == "L" :
            self.dependencies.append((self.stack[-1],self.stack[-2]))
            last = self.stack[-1]
            self.stack = self.stack[0:-2]
            self.stack.append(last)
        elif transition == "RA" or transition == "R":
...
def minibatch_parse(sentences, model, batch_size):
    ...
    ### END YOUR CODE
    dependencies = list(range(0,len(sentences)))
    partial_parses = [PartialParse(sentence) for sentence in sentences]

    unfinished_parses = partial_parses[0:batch_size]
    #print(unfinished_parses[0].buffer)
    index = list(range(0,batch_size))
    nowprocess = 0
    # while(nowprocess != len(sentences)):
    #     transitions = model.predict(unfinished_parses)
    #     print(transitions)
    #     for i in range(0,len(unfinished_parses)):
    #         dependencies.append(unfinished_parses[i].parse(transitions[i]))
    #         print(dependencies[0])
    #     if nowprocess == len(sentences)+1:
    #         break
    #     if nowprocess + batch_size <= len(sentences):
    #         unfinished_parses = partial_parses[nowprocess+1:nowprocess+1+batch_size]
    #         nowprocess += batch_size
    #     else:
    #         unfinished_parses = partial_parses[nowprocess+1:-1]
    #         nowprocess = len(sentences)+1
    while(nowprocess != len(sentences)):
        #a+=1
        transitions = model.predict(unfinished_parses)
        #transitions = ["".join(char) for char in transitions]
        toprocess = []
        for i in range(0,min(len(unfinished_parses),len(sentences)-nowprocess)):
            #print(i)
            unfinished_parses[i].parse(transitions[i])
            # if i ==1 :
            #     print(unfinished_parses[i].stack)
            #     print(unfinished_parses[i].buffer)
            #     print(transitions[i])
            if len(unfinished_parses[i].buffer) == 0 and len(unfinished_parses[i].stack) == 1:
                toprocess.append(i)

        nowprocess += len(toprocess)
        toprocess_sorted = sorted(toprocess, reverse=True)
        for tp in toprocess_sorted:
            dependencies[index[tp]] = unfinished_parses[tp].dependencies
            index.pop(tp)
            unfinished_parses.pop(tp)

        for j in range(0,len(toprocess)):
            if nowprocess == len(sentences):
                break
            index.append(max(index) + 1)
            if index[-1] < len(sentences):
                unfinished_parses.append(partial_parses[index[-1]])
