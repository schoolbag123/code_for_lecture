...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        #好像应该用empty(问如何使用初始化函数的时候ai顺手把东西全输出了)
    ...
    def forward(self, w):
        ...
        # print(f"shape of embed_to_hidden_weight:{self.embed_to_hidden_weight.shape}")
        # print(f"shape of embed_to_hidden_bias:{self.embed_to_hidden_bias.shape}")
        h = x @ self.embed_to_hidden_weight + self.embed_to_hidden_bias
        # print(f"shape of h:{h.shape}")
        h = torch.where(h >=0,h,0)
        h = self.dropout(h)
        # print(f"shape of h:{h.shape}")
        # print(f"shape of hidden_to_logits_weight:{self.hidden_to_logits_weight.shape}")
        # print(f"shape of hidden_to_logits_bias:{self.hidden_to_logits_bias.shape}")
